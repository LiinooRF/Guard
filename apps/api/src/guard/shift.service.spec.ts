...
const sinAlertas = () => ({}) as unknown as AlertsService;

function servicio(query: jest.Mock) {
  return new GuardService({ manager: { query } } as unknown as TenantContextService, sinCorreo(), sinReglas(), sinEscalamiento(), sinAlertas());
}
